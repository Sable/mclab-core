...
private TranslationProblem makeProblem(RecognitionException e) {
    return makeProblem(getTokenNames(), e);
}

/* Convert a RecognitionException into a TranslationException. */
private TranslationProblem makeProblem(String[] tokenNames, RecognitionException e) {
    //change column to 1-based
    return new TranslationProblem(e.getOffendingToken().getLine(), e.getOffendingToken().getCharPositionInLine() + 1, e.getMessage());
}
/*
public static String translate(String text, int baseLine, int baseCol, OffsetTracker offsetTracker, List<TranslationProblem> problems) {
    return translate(new ANTLRInputStream(text), baseLine, baseCol, offsetTracker, problems);
}

public static String translate(ANTLRInputStream in, int baseLine, int baseCol, OffsetTracker offsetTracker, List<TranslationProblem> problems) {
    in.setLine(baseLine);
...
private final List<TranslationProblem> problems = new ArrayList<TranslationProblem>();
...
private String insertDeletedComments() {
    ...
    Token prevTok = getTokenStream().LT(-1);
    if(prevTok.getType() != LINE_TERMINATOR) {
        ...
        if(chompedPrecedingEOFPos.getLine() == 1) {
            col += getTokenStream().LT(-1).getCharPositionInLine();
        }
...
compound_stmt_header_sep :
     stmt_separator t_FILLER? #NO_COMMA
  |  t_FILLER stmt_separator t_FILLER? #NO_COMMA //don't insert a comma if we don't have to
  ...
  | {isCompoundStmtHeaderSeparator()}? { offsetTracker.recordOffsetChange(0, -1); offsetTracker.advanceInLine(1); } t_FILLER #COMMA_INSERT
  //TODO VSD
  ;
...
function_beginning :
     ...
     (((dt_FILLER | dt_LINE_TERMINATOR | dt_COMMENT | dt_BRACKET_COMMENT) { appendToLeadingComments($text, $start.getCharPositionInLine()); }) )*
     //TODO VSD
  ;
...
element_separator_list :
     t_FILLER? t_COMMA t_FILLER? #ECHO //just echo
  ...
  |  {isElementSeparator()}? { offsetTracker.recordOffsetChange(0, -1); offsetTracker.advanceInLine(1); } t_FILLER #COMMA_INSERT2
  //TODO VSD
  ;
...
//t_COMMENT : COMMENT { offsetTracker.advanceByTextSize($text); } -> template(revised={$text.startsWith("\%\%") ? ("\% " + $text.substring(2)) : $text}) "<revised>";
t_COMMENT : COMMENT { offsetTracker.advanceByTextSize($text); } #REVISE;
//TODO VSD
...
//dt_COMMENT : COMMENT { offsetTracker.recordOffsetChange($text, $COMMENT.pos + 1, false); } -> template(revised={$text.startsWith("\%\%") ? ("\% " + $text.substring(2)) : $text}) "<revised>";
dt_COMMENT : COMMENT { offsetTracker.recordOffsetChange($text, $COMMENT.pos + 1, false); } #REVISE2;
//TODO VSD
