...
private octave.TranslationProblem makeProblem(RecognitionException e) {
...
private octave.TranslationProblem makeProblem(String[] tokenNames, RecognitionException e) {
    ...
    return new octave.TranslationProblem(e.line, e.charPositionInLine + 1, getErrorMessage(e, tokenNames));
}

public static String translate(String text, int baseLine, int baseCol, OffsetTracker offsetTracker, List<octave.TranslationProblem> problems) {
    return translate(new ANTLRStringStream(text), baseLine, baseCol, offsetTracker, problems);
}

public static String translate(ANTLRStringStream in, int baseLine, int baseCol, OffsetTracker offsetTracker, List<octave.TranslationProblem> problems) {
    in.setLine(baseLine);
