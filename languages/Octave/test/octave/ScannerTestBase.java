...
/** 
 * Parent class of the generated OctaveScannerTests class.
 * Provides helper methods to keep OctaveScannerTests short.
 */
class ScannerTestBase extends TestCase {
    ...
    static void checkScan(OctaveLexer lexer, List<Symbol> symbols, TextPosition exceptionPos) throws IOException {
        ...
        if(exceptionPos == null && tokenNum < actualTokens.size()) {
            ...
            int expectedType = OctaveParser.EOF;
            fail("Token #" + tokenNum + ": incorrect token type - " +
                    "expected: " + expectedType + " (EOF) " +
                    "but was: " + actualType + " (" + OctaveParser.tokenNames[actualType] + ")");
        }
        if(lexer.hasProblem()) {
            TranslationProblem prob = lexer.getProblems().get(0);
            assertEquals(prob.getMessage(), new TextPosition(prob.getLine(), prob.getColumn()), exceptionPos);
    ...
    static OctaveLexer getLexer(String filename) throws IOException {
        BufferedReader in = new BufferedReader(new FileReader(filename));
        OctaveLexer lexer = new OctaveLexer(new ANTLRReaderStream(in));
        return lexer;
    }
    ...
    private static Symbol parseSymbol(String line) 
        ...
        int id = OctaveParser.class.getField(idString).getInt(null); //null since the field is static
    ...
    public static void assertEquals(String msg, Token actual, Symbol expected) throws IOException {
        ...
        if(actualId != expectedId) {
            fail(msg + ": incorrect token type - " +
                    "expected: " + expectedId + " (" + OctaveParser.tokenNames[expectedId] + ") " +
                    "but was: " + actualId + " (" + OctaveParser.tokenNames[actualId] + ")");
        }
        ...
        if(((actualValue == null || expectedValue == null) && (actualValue != expectedValue)) || 
            ...
            fail(msg + " - " + OctaveParser.tokenNames[actualId] + ": incorrect token value - " +
    ...
    protected static String getTokenString(Token tok) {
        return OctaveParser.tokenNames[tok.getType()] + "(" + tok.getText() + ")";
    }
    ...
    static class Symbol {
        ...
        public String toString() {
            return OctaveParser.tokenNames[type] + "[" + type + "] - (" + startLine + ", " + startCol + ") to (" + endLine + ", " + endCol + ") = " + text;
        }
    }
}
