...
public class McLint {
	...
	private static List<LintAnalysis> getBuiltinAnalyses(Project project) {
		return ImmutableList.<LintAnalysis> builder()
				...
				.add(new UnusedVar(project)).add(new Shadowing(project))
				.add(new UnreachableCode(project)).build();
	}
	...
	private static List<LintAnalysis> getAllAnalyses(Project project) {
		return ImmutableList.<LintAnalysis> builder()
				...
				.addAll(getPluginAnalyses(project)).build();
	}
	...
	private static boolean prompt(Message message, String prompt) {
		...
		return decision.isEmpty() || decision.startsWith("y")
				|| decision.startsWith("Y");
	}
	...
	private static void registerBuiltinListeners(final Lint lint) {
		...
		lint.registerListenerForMessageCode(
				"SHADOW_BUILTIN",
				message -> {
					...
					String newName = System.console().readLine(
							"    rename %s to: ", node.getID());
					RefactoringContext context = lint
							.getLayoutPreservingRefactoringContext();
					Refactoring rename = Refactorings.renameVariable(context,
							node, newName);
