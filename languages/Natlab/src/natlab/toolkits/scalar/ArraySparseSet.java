...
/**
 * Adapted from soot.toolkits.scalar.ArraySparseSet.java
 * Change Log:
 *  - 2008.06 by Jun Li
 *  	- adding another different(othe,gen,dest) 
 *  	- adding toString()
 */
...
public class ArraySparseSet extends AbstractFlowSet
{
  ...
  public void union(FlowSet otherFlow, FlowSet destFlow)
    ...
    /*
     * An overload function to original different(other,dest)
     * It's designed specific for reaching-definition analysis (ReachingDefs)
     *     dest =  gen.def + (other - gen.def )
     * 
     * 	   # (other - gen.def): {all definitions of other-set 
	 *			                 that are for same variables of new gen-set}
 
     */
    public void difference(FlowSet otherFlow, FlowSet genFlow, FlowSet destFlow)
    {
    	
		ArraySparseSet other = (ArraySparseSet) otherFlow;
		ArraySparseSet gen = (ArraySparseSet) genFlow;
		ArraySparseSet dest = (ArraySparseSet) destFlow;
		
    	ArraySparseSet workingSet = new ArraySparseSet();
    	gen.copy(workingSet);
    	...
    	for(int i=0; i<other.numElements; i++) {
    		...
    		// this one isn't
    		if(preDefined == false)
    			workingSet.add(other.elements[i]);
    	}
    ...
    /*
     * Override natlab.toolkits.scalar.AbstractFlowSet#toString(),
     * just to output the node-ID instead of whole node.
     */
