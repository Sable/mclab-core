...
public class TIRToMcSAFIRWithoutTemp extends AbstractTIRLocalRewrite implements
		TamerPlusTransformation {
	...
	public void transform(TransformationEngine engine) {
		...
		fTIRToRawASTTable = analysisEngine
	...
	public ASTNode getReplacementNode(TIRNode node) {
		if (fTIRToRawASTTable.containsKey(node)) {
			...
			if (replacementNode != null) {
				...
				throw new NoSuchElementException("Entry for TIRNode " + node
						+ " is null in TIR to AST table");
			}
		} else {
			throw new NoSuchElementException("Key TIRNode " + node
					+ " is not present in TIR to AST table");
		}
	}

	private boolean nodeDefinesTmpVariables(TIRNode node) {
		Set<String> definedVariablesNames = fDefinedVariablesNameCollector
